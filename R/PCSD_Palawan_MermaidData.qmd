...
```{r}
#| label: readData
dat <- read_csv("../data/2017_AlbinaReef_Dumaran.csv", trim_ws = TRUE)
```
...
```{r}
dat <- dat |> 
  dplyr::select(-Month, -Day, -Benthic_attribute, -Growth_form, - LIT_measurement) |> 
  group_by(Site, Latitude, Longitude, Exposure, Reef_slope, Year, Transect_number, Benthic_category) |> 
  count(name = "COUNT") |> 
