...
model1 |> 
  ...
  plot() |> 
  _[[1]] #+
  #geom_point(data = dat, aes(y = COUNT/TOTAL, x = site_name), inherit.aes = FALSE)
```
...
```{r}
form2 <- bf(COUNT | trials(TOTAL) ~ site_name + (1|transect_id), family = beta_binomial(link = "logit"))
```
...
```
```{r}
reefs <- read_sf("../PCSD-Repository/data/GIS/reef_500_poly.shp")
ggplot() +
  geom_sf(data = aus) +
