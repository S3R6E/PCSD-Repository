...
```{r}
dat <- read_csv("../PCSD-Repository/data/Balabac.csv", trim_ws = TRUE)
```
...
```{r}
dat <- read_csv("../PCSD-Repository/data/Quezon.csv", trim_ws = TRUE)
```
...
```{r}
form3 <- bf(COUNT | trials(TOTAL) ~ site_name + (1|transect_id),
           family = binomial(link = "logit"))
```
...
```
```{r}
model3 |> conditional_effects() |> 
  plot() |> 
  _[[1]] #+
  #geom_point(data = dat_hc_BL, aes(y = COUNT/TOTAL, x = site_name, inherit.aes = FALSE))
#model3 <- update(model1, sample_prior = "yes")
```

```{r}
model3 <-
  model3 |> update(sample_prior = "yes")
```
...
### Define priors
```{r}
model3 |> SUYR_prior_and_posterior()
```

### Fit prior only model

```{r}
model3$fit |> stan_trace()
```
```{r}
model3$fit |> stan_ac()
```
```{r}
model3$fit |> stan_rhat()
```
```{r}
model3$fit |> stan_ess()
```
...
### Posterior probability checks
```{r}
model3 |> pp_check(type = 'dens_overlay', ndraws = 100)
```

### Model validation

```{r}
resids <- model3 |> make_brms_dharma_res(integerResponse = FALSE)
```
...
```{r}
form3 <- bf(COUNT | trials(TOTAL) ~ site_name + (1|transect_id), family = beta_binomial(link = "logit"))
```
...
```{r}
model4 |> conditional_effects() |> 
  plot() |> 
  _[[1]]
```

```{r}
model4 <- update(model4, sample_prior = "yes")
```
```{r}
model4 |> conditional_effects() |> 
  plot() |> 
...
```{r}
model4 |> SUYR_prior_and_posterior()
```
```{r}
model4$fit |> stan_trace()
```
```{r}
model4$fit |> stan_ac()
```
```{r}
model4$fit |> stan_rhat()
```
```{r}
model4$fit |> stan_ess()
```
...
### Posterior probability checks
```{r}
model4 |> pp_check(type = 'dens_overlay', ndraws = 100)
```

```{r}
resids <- model4 |> make_brms_dharma_res(integerResponse = FALSE)
```
...
## Pairwise constrasts
```{r}
model4 |> emmeans(~site_name, type = 'response') |> 
  pairs()
